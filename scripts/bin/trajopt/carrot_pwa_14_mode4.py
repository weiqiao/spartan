...
# mode 4: neither left finger nor right finger in contact
# F1, F1t, F2, F2t disappear
...
def dh(X,F,params):
	...
	# constant term
	coeff_c = (- Fn*r0*np.sin(theta) + Ft*(r-r0*np.cos(theta)))/I
	coeff_c -= coeff_x.dot(X) + coeff_u.dot(F)
...
def constraints(X,F,params):
	...
	### NEW (MODE 4 ONLY): finger 1 lost contact
	# -F1 <= 0
	cons = np.zeros(len_h)
	cons[6] = -1
	h2 = 0
	...
	### NEW (MODE 4 ONLY): finger 2 lost contact
	# -F2 <= 0
	cons = np.zeros(len_h)
	cons[8] = -1
	h2 = 0
