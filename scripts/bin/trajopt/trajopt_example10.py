...
# different from example 9: final angle is 120 degrees
...
class TrajectoryOptimization(mp.MathematicalProgram):
	def add_dynamics_constraints(self, params, pos_init, pos_final):
		...
		for t in range(T):
			...
			self.AddLinearConstraint(phi <= np.pi*3/4)
...
def visualize(X,F,t):
    ...
    t += 1
    fig.savefig('trajopt_example10_fig_latest/carrot_%d.png'%t, dpi=100)
    plt.close()
    return fig
...
if __name__=="__main__":
	...
	if SAVE_STATE_AND_CONTROL:
		...
		pickle.dump( state_and_control, open("trajopt_example10_latest.p","wb"))
