...
# mode 3: left finger not in contact, right finger in contact
# F1 and F1t disappear
...
def df(X,F,params):
	...
	coeff_u = np.array([df_dF1,df_dF1t,df_dF2,df_dF2t,df_dFn,df_dFt,df_dphi])

	# constant term
	coeff_c = (- F2*np.sin(phi) + F2t*np.cos(phi) + Ft) / m
...
def dg(X,F,params):
	...
	coeff_u = np.array([dg_dF1,dg_dF1t,dg_dF2,dg_dF2t,dg_dFn,dg_dFt,dg_dphi])

	# constant term
	coeff_c = (F2*np.cos(phi) + F2t*np.sin(phi) + Fn - m*g) / m
	coeff_c -= coeff_x.dot(X) + coeff_u.dot(F)
...
def dh(X,F,params):
	...
	coeff_u = np.array([dh_dF1,dh_dF1t,dh_dF2,dh_dF2t,dh_dFn,dh_dFt,dh_dphi])

	# constant term
	coeff_c = (F2*r0*np.sin(phi-theta) + F2t*(r-r0*np.cos(theta-phi)) - Fn*r0*np.sin(theta) + Ft*(r-r0*np.cos(theta)))/I
	coeff_c -= coeff_x.dot(X) + coeff_u.dot(F)
...
def constraints(X,F,params):
	...
	### NEW (MODE 3 ONLY): finger 1 lost contact
	# -F1 <= 0
	cons = np.zeros(len_h)
	cons[6] = -1
	h2 = 0
