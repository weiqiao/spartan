...
# finger 1 initial and final contact points are different 
# different from example 6: finger 2 has friction
...
def visualize(X,F,t):
    ...
    t += 1
    fig.savefig('trajopt_example7_fig/carrot_%d.png'%t, dpi=100)
    plt.close()
    return fig
...
def draw_force(ax,X,F):
	...
	ax.arrow(x_F2_tp,y_F2_tp,dx_F2_tp,dy_F2_tp,color=(1,0,1),head_width=0.05, head_length=0.1)
	ax.arrow(x_F2_tm,y_F2_tm,dx_F2_tm,dy_F2_tm,color=(1,0,1),head_width=0.05, head_length=0.1)
...
if __name__=="__main__":
	...
	theta_final = np.pi/2
	pos_final = np.array([DistanceCentroidToCoM*np.sin(theta_final)-r*theta_final,r-DistanceCentroidToCoM*np.cos(theta_final),theta_final,0,0,0,0.3])
	initial_guess = {}
