...
# derived from example 11.
# different from example 11: assume the left finger is fixed. do not use LCP constraints.
# the dim of states is reduced to 6 and the dim of control is reduced to 8.
...
class TrajectoryOptimization(mp.MathematicalProgram):
	def add_dynamics_constraints(self, params, pos_init, pos_final):
		...
		# F = [F1, F1t, F2, F2t, Fn, Ft, phi, omega]
		...
		for t in range(T):
			...
			F1, F1t, F2, F2t, Fn, Ft, phi, omega = F_over_time[t,:]
